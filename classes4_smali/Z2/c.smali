.class public final LZ2/c;
.super Ljava/lang/Object;
.source "SourceFile"

# interfaces
.implements Ljava/lang/Thread$UncaughtExceptionHandler;


# instance fields
.field public final a:[Ljava/lang/String;

.field public final b:Ljava/lang/String;

.field public final c:Landroid/content/Context;

.field public final d:Ljava/lang/Thread$UncaughtExceptionHandler;

.field public final e:LZ2/b;

.field public final f:LA4/r;

.field public g:Z


# direct methods
.method public constructor <init>(Landroid/content/Context;Ljava/lang/Thread$UncaughtExceptionHandler;LZ2/b;)V
    .registers 7

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    const-string v0, "df"

    const-string v1, "logcat -b events -v threadtime -v printable -v uid -d *:v"

    const-string v2, "cat /proc/meminfo"

    filled-new-array {v1, v2, v0}, [Ljava/lang/String;

    move-result-object v0

    iput-object v0, p0, LZ2/c;->a:[Ljava/lang/String;

    const/4 v0, 0x0

    iput-boolean v0, p0, LZ2/c;->g:Z

    iput-object p1, p0, LZ2/c;->c:Landroid/content/Context;

    iput-object p2, p0, LZ2/c;->d:Ljava/lang/Thread$UncaughtExceptionHandler;

    iput-object p3, p0, LZ2/c;->e:LZ2/b;

    new-instance p2, Ljava/lang/StringBuilder;

    invoke-direct {p2}, Ljava/lang/StringBuilder;-><init>()V

    invoke-virtual {p1}, Landroid/content/Context;->getApplicationInfo()Landroid/content/pm/ApplicationInfo;

    move-result-object p3

    iget-object p3, p3, Landroid/content/pm/ApplicationInfo;->dataDir:Ljava/lang/String;

    const-string v1, "/exception/"

    invoke-static {p2, p3, v1}, Landroidx/concurrent/futures/a;->n(Ljava/lang/StringBuilder;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object p2

    iput-object p2, p0, LZ2/c;->b:Ljava/lang/String;

    const-string p3, "Diagmon Logger Init"

    invoke-static {p3}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    new-instance p3, Ljava/lang/StringBuilder;

    const-string v1, "CRASH_LOG_PATH : "

    invoke-direct {p3, v1}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p3, p2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v1, "diagmon.log"

    invoke-virtual {p3, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p3

    invoke-static {p3}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    new-instance p3, Ljava/lang/StringBuilder;

    const-string v1, "EVENT_LOG_PATH : "

    invoke-direct {p3, v1}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p3, p2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v1, "diagmon_event.log"

    invoke-virtual {p3, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p3

    invoke-static {p3}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    new-instance p3, Ljava/lang/StringBuilder;

    const-string v1, "THREAD_STACK_LOG_PATH : "

    invoke-direct {p3, v1}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p3, p2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v1, "diagmon_thread.log"

    invoke-virtual {p3, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p3

    invoke-static {p3}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    new-instance p3, Ljava/lang/StringBuilder;

    const-string v1, "MEMORY_LOG_PATH : "

    invoke-direct {p3, v1}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p3, p2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v1, "diagmon_memory.log"

    invoke-virtual {p3, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p3

    invoke-static {p3}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    new-instance p3, Ljava/lang/StringBuilder;

    const-string v1, "STORAGE_LOG_PATH : "

    invoke-direct {p3, v1}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p3, p2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v1, "diagmon_storage.log"

    invoke-virtual {p3, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p3

    invoke-static {p3}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    const/4 p3, 0x2

    const/4 v1, 0x1

    :try_start_a0
    invoke-virtual {p1}, Landroid/content/Context;->getPackageManager()Landroid/content/pm/PackageManager;

    move-result-object p1

    const-string v2, "com.sec.android.diagmonagent"

    invoke-virtual {p1, v2, v0}, Landroid/content/pm/PackageManager;->getPackageInfo(Ljava/lang/String;I)Landroid/content/pm/PackageInfo;

    move-result-object p1

    iget p1, p1, Landroid/content/pm/PackageInfo;->versionCode:I
    :try_end_ac
    .catch Landroid/content/pm/PackageManager$NameNotFoundException; {:try_start_a0 .. :try_end_ac} :catch_b4

    const v0, 0x23c34600

    if-ge p1, v0, :cond_b3

    move v0, v1

    goto :goto_b4

    :cond_b3
    move v0, p3

    :catch_b4
    :goto_b4
    const-string p1, "fatal exception"

    if-eq v0, v1, :cond_c9

    if-eq v0, p3, :cond_bb

    goto :goto_d4

    :cond_bb
    new-instance p3, LA4/r;

    const/16 v0, 0x13

    invoke-direct {p3, v0}, LA4/r;-><init>(I)V

    iput-object p2, p3, LA4/r;->b:Ljava/lang/Object;

    iput-object p1, p3, LA4/r;->c:Ljava/lang/Object;

    iput-object p3, p0, LZ2/c;->f:LA4/r;

    goto :goto_d4

    :cond_c9
    new-instance p2, LA4/r;

    const/16 p3, 0x13

    invoke-direct {p2, p3}, LA4/r;-><init>(I)V

    iput-object p1, p2, LA4/r;->c:Ljava/lang/Object;

    iput-object p2, p0, LZ2/c;->f:LA4/r;

    :goto_d4
    return-void
.end method

.method public static b()Ljava/lang/String;
    .registers 8

    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    invoke-static {}, Ljava/lang/Thread;->getAllStackTraces()Ljava/util/Map;

    move-result-object v1

    invoke-interface {v1}, Ljava/util/Map;->keySet()Ljava/util/Set;

    move-result-object v1

    invoke-interface {v1}, Ljava/util/Set;->iterator()Ljava/util/Iterator;

    move-result-object v1

    :goto_11
    invoke-interface {v1}, Ljava/util/Iterator;->hasNext()Z

    move-result v2

    if-eqz v2, :cond_64

    invoke-interface {v1}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/Thread;

    invoke-virtual {v2}, Ljava/lang/Thread;->getStackTrace()[Ljava/lang/StackTraceElement;

    move-result-object v3

    array-length v4, v3

    const/4 v5, 0x1

    if-ge v4, v5, :cond_2b

    const-string v2, "no StackTraceElement"

    invoke-static {v2}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    goto :goto_11

    :cond_2b
    const-string v4, "Thread ID : "

    invoke-virtual {v0, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v2}, Ljava/lang/Thread;->getId()J

    move-result-wide v4

    invoke-virtual {v0, v4, v5}, Ljava/lang/StringBuilder;->append(J)Ljava/lang/StringBuilder;

    const-string v4, ", Thread\'s name : "

    invoke-virtual {v0, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v2}, Ljava/lang/Thread;->getName()Ljava/lang/String;

    move-result-object v2

    invoke-virtual {v0, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v2, "\n"

    invoke-virtual {v0, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    array-length v4, v3

    const/4 v5, 0x0

    :goto_4a
    if-ge v5, v4, :cond_60

    aget-object v6, v3, v5

    const-string v7, "\t at "

    invoke-virtual {v0, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v6}, Ljava/lang/StackTraceElement;->toString()Ljava/lang/String;

    move-result-object v6

    invoke-virtual {v0, v6}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v0, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    add-int/lit8 v5, v5, 0x1

    goto :goto_4a

    :cond_60
    invoke-virtual {v0, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    goto :goto_11

    :cond_64
    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v1

    if-eqz v1, :cond_70

    const-string v0, "No data"

    :cond_70
    return-object v0
.end method

.method public static c(Landroid/content/Context;Ljava/lang/String;)Ljava/lang/String;
    .registers 4

    invoke-static {p0}, Lj2/b;->p(Landroid/content/Context;)Landroid/content/pm/PackageInfo;

    move-result-object p0

    if-eqz p0, :cond_49

    new-instance v0, Ljava/lang/StringBuilder;

    const-string v1, "=========================================\nService version   : "

    invoke-direct {v0, v1}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    iget-object p0, p0, Landroid/content/pm/PackageInfo;->versionName:Ljava/lang/String;

    const-string v1, "\nDiagMonSA SDK version : 6.05.068\n=========================================\n"

    invoke-static {v0, p0, v1}, Landroidx/concurrent/futures/a;->n(Ljava/lang/StringBuilder;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object p0

    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0, p0}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    :try_start_1a
    invoke-static {}, Ljava/lang/Runtime;->getRuntime()Ljava/lang/Runtime;

    move-result-object p0

    invoke-virtual {p0, p1}, Ljava/lang/Runtime;->exec(Ljava/lang/String;)Ljava/lang/Process;

    move-result-object p0

    new-instance p1, Ljava/io/BufferedReader;

    new-instance v1, Ljava/io/InputStreamReader;

    invoke-virtual {p0}, Ljava/lang/Process;->getInputStream()Ljava/io/InputStream;

    move-result-object p0

    invoke-direct {v1, p0}, Ljava/io/InputStreamReader;-><init>(Ljava/io/InputStream;)V

    invoke-direct {p1, v1}, Ljava/io/BufferedReader;-><init>(Ljava/io/Reader;)V

    :goto_30
    invoke-virtual {p1}, Ljava/io/BufferedReader;->readLine()Ljava/lang/String;

    move-result-object p0

    if-eqz p0, :cond_44

    invoke-virtual {v0, p0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string p0, "\n"

    invoke-virtual {v0, p0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;
    :try_end_3e
    .catch Ljava/io/IOException; {:try_start_1a .. :try_end_3e} :catch_3f

    goto :goto_30

    :catch_3f
    const-string p0, "IOException occurred during getCrashLog"

    invoke-static {p0}, Lcom/bumptech/glide/c;->o(Ljava/lang/String;)V

    :cond_44
    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p0

    return-object p0

    :cond_49
    const-string p0, ""

    return-object p0
.end method

.method public static d(Ljava/lang/String;Ljava/lang/String;)Ljava/io/File;
    .registers 4

    new-instance v0, Ljava/io/File;

    invoke-direct {v0, p0}, Ljava/io/File;-><init>(Ljava/lang/String;)V

    invoke-virtual {v0}, Ljava/io/File;->exists()Z

    move-result v1

    if-nez v1, :cond_e

    invoke-virtual {v0}, Ljava/io/File;->mkdirs()Z

    :cond_e
    invoke-virtual {v0}, Ljava/io/File;->isDirectory()Z

    move-result v0

    if-eqz v0, :cond_2c

    new-instance v0, Ljava/io/File;

    const-string v1, "/"

    invoke-static {p0, v1, p1}, Landroidx/concurrent/futures/a;->l(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object p0

    invoke-direct {v0, p0}, Ljava/io/File;-><init>(Ljava/lang/String;)V

    :try_start_1f
    invoke-virtual {v0}, Ljava/io/File;->createNewFile()Z
    :try_end_22
    .catch Ljava/io/IOException; {:try_start_1f .. :try_end_22} :catch_23

    goto :goto_2d

    :catch_23
    move-exception p0

    invoke-virtual {p0}, Ljava/lang/Throwable;->getLocalizedMessage()Ljava/lang/String;

    move-result-object p0

    invoke-static {p0}, Ll0/a;->d(Ljava/lang/String;)V

    goto :goto_2d

    :cond_2c
    const/4 v0, 0x0

    :goto_2d
    return-object v0
.end method


# virtual methods
.method public final a()V
    .registers 5

    iget-object p0, p0, LZ2/c;->f:LA4/r;

    invoke-static {}, LZ2/e;->b()V

    invoke-static {}, LS0/e;->u()LS0/e;

    move-result-object v0

    new-instance v1, LF/c;

    sget-object v2, LZ2/e;->a:LZ2/b;

    sget-object v3, LZ2/e;->b:Landroid/os/Bundle;

    invoke-direct {v1, v2, v3, p0}, LF/c;-><init>(LZ2/b;Landroid/os/Bundle;LA4/r;)V

    invoke-virtual {v0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    invoke-static {v1}, LS0/e;->t(LY2/a;)V

    new-instance p0, Ljava/lang/StringBuilder;

    const-string v0, "[Falcon_DiagMonSDK][3]["

    invoke-direct {p0, v0}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    const-string v0, "c"

    invoke-virtual {p0, v0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v0, "]"

    invoke-virtual {p0, v0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p0

    invoke-static {p0}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    return-void
.end method

.method public final e(Ljava/io/File;Ljava/lang/Throwable;Ljava/lang/String;)V
    .registers 7

    if-eqz p1, :cond_64

    invoke-virtual {p1}, Ljava/io/File;->exists()Z

    move-result v0

    if-eqz v0, :cond_64

    if-nez p2, :cond_b

    goto :goto_64

    :cond_b
    :try_start_b
    new-instance v0, Ljava/io/FileOutputStream;

    iget-boolean v1, p0, LZ2/c;->g:Z

    invoke-direct {v0, p1, v1}, Ljava/io/FileOutputStream;-><init>(Ljava/io/File;Z)V
    :try_end_12
    .catch Ljava/io/IOException; {:try_start_b .. :try_end_12} :catch_34
    .catch Ljava/lang/OutOfMemoryError; {:try_start_b .. :try_end_12} :catch_32

    :try_start_12
    new-instance p1, Ljava/io/PrintStream;

    const-string v1, "UTF-8"

    const/4 v2, 0x1

    invoke-direct {p1, v0, v2, v1}, Ljava/io/PrintStream;-><init>(Ljava/io/OutputStream;ZLjava/lang/String;)V
    :try_end_1a
    .catchall {:try_start_12 .. :try_end_1a} :catchall_36

    :try_start_1a
    iput-boolean v2, p0, LZ2/c;->g:Z

    invoke-static {p3}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result p0

    if-eqz p0, :cond_28

    invoke-virtual {p2, p1}, Ljava/lang/Throwable;->printStackTrace(Ljava/io/PrintStream;)V

    goto :goto_2b

    :catchall_26
    move-exception p0

    goto :goto_38

    :cond_28
    invoke-virtual {p1, p3}, Ljava/io/PrintStream;->println(Ljava/lang/String;)V
    :try_end_2b
    .catchall {:try_start_1a .. :try_end_2b} :catchall_26

    :goto_2b
    :try_start_2b
    invoke-virtual {p1}, Ljava/io/PrintStream;->close()V
    :try_end_2e
    .catchall {:try_start_2b .. :try_end_2e} :catchall_36

    :try_start_2e
    invoke-virtual {v0}, Ljava/io/FileOutputStream;->close()V
    :try_end_31
    .catch Ljava/io/IOException; {:try_start_2e .. :try_end_31} :catch_34
    .catch Ljava/lang/OutOfMemoryError; {:try_start_2e .. :try_end_31} :catch_32

    goto :goto_63

    :catch_32
    move-exception p0

    goto :goto_4a

    :catch_34
    move-exception p0

    goto :goto_57

    :catchall_36
    move-exception p0

    goto :goto_41

    :goto_38
    :try_start_38
    invoke-virtual {p1}, Ljava/io/PrintStream;->close()V
    :try_end_3b
    .catchall {:try_start_38 .. :try_end_3b} :catchall_3c

    goto :goto_40

    :catchall_3c
    move-exception p1

    :try_start_3d
    invoke-virtual {p0, p1}, Ljava/lang/Throwable;->addSuppressed(Ljava/lang/Throwable;)V

    :goto_40
    throw p0
    :try_end_41
    .catchall {:try_start_3d .. :try_end_41} :catchall_36

    :goto_41
    :try_start_41
    invoke-virtual {v0}, Ljava/io/FileOutputStream;->close()V
    :try_end_44
    .catchall {:try_start_41 .. :try_end_44} :catchall_45

    goto :goto_49

    :catchall_45
    move-exception p1

    :try_start_46
    invoke-virtual {p0, p1}, Ljava/lang/Throwable;->addSuppressed(Ljava/lang/Throwable;)V

    :goto_49
    throw p0
    :try_end_4a
    .catch Ljava/io/IOException; {:try_start_46 .. :try_end_4a} :catch_34
    .catch Ljava/lang/OutOfMemoryError; {:try_start_46 .. :try_end_4a} :catch_32

    :goto_4a
    const-string p1, "OutOfMemoryError Exception occurred during writeLogFile"

    invoke-static {p1}, Lcom/bumptech/glide/c;->o(Ljava/lang/String;)V

    invoke-virtual {p0}, Ljava/lang/Throwable;->getMessage()Ljava/lang/String;

    move-result-object p0

    invoke-static {p0}, Lcom/bumptech/glide/c;->o(Ljava/lang/String;)V

    goto :goto_63

    :goto_57
    const-string p1, "IOException occurred during writeLogFile"

    invoke-static {p1}, Lcom/bumptech/glide/c;->o(Ljava/lang/String;)V

    invoke-virtual {p0}, Ljava/lang/Throwable;->getMessage()Ljava/lang/String;

    move-result-object p0

    invoke-static {p0}, Lcom/bumptech/glide/c;->o(Ljava/lang/String;)V

    :goto_63
    return-void

    :cond_64
    :goto_64
    const-string p0, "Failed to write log into file"

    invoke-static {p0}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    return-void
.end method

.method public final f()V
    .registers 6

    new-instance v0, Ljava/io/File;

    iget-object v1, p0, LZ2/c;->b:Ljava/lang/String;

    invoke-direct {v0, v1}, Ljava/io/File;-><init>(Ljava/lang/String;)V

    invoke-virtual {v0}, Ljava/io/File;->exists()Z

    move-result v1

    if-nez v1, :cond_13

    const-string p0, "The directory doesn\'t exist."

    invoke-static {p0}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    return-void

    :cond_13
    invoke-virtual {v0}, Ljava/io/File;->listFiles()[Ljava/io/File;

    move-result-object v0

    if-eqz v0, :cond_2f

    array-length v1, v0

    const/4 v2, 0x0

    :goto_1b
    if-ge v2, v1, :cond_2f

    aget-object v3, v0, v2

    invoke-virtual {v3}, Ljava/io/File;->isDirectory()Z

    move-result v4

    if-eqz v4, :cond_29

    invoke-virtual {p0}, LZ2/c;->f()V

    goto :goto_2c

    :cond_29
    invoke-virtual {v3}, Ljava/io/File;->delete()Z

    :goto_2c
    add-int/lit8 v2, v2, 0x1

    goto :goto_1b

    :cond_2f
    return-void
.end method

.method public final g()V
    .registers 5

    new-instance v0, Ljava/io/File;

    iget-object p0, p0, LZ2/c;->b:Ljava/lang/String;

    invoke-direct {v0, p0}, Ljava/io/File;-><init>(Ljava/lang/String;)V

    invoke-virtual {v0}, Ljava/io/File;->listFiles()[Ljava/io/File;

    move-result-object p0

    invoke-virtual {v0}, Ljava/io/File;->exists()Z

    move-result v0

    if-nez v0, :cond_17

    const-string p0, "The directory doesn\'t exist."

    invoke-static {p0}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    return-void

    :cond_17
    const/4 v0, 0x0

    :goto_18
    array-length v1, p0

    if-ge v0, v1, :cond_38

    const-string v1, "[Falcon_DiagMonSDK][2]["

    const-string v2, "c"

    const-string v3, "]"

    invoke-static {v1, v2, v3}, LG2/u;->n(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v1

    aget-object v2, p0, v0

    invoke-virtual {v2}, Ljava/io/File;->getName()Ljava/lang/String;

    move-result-object v2

    invoke-virtual {v1, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v1

    invoke-static {v1}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    add-int/lit8 v0, v0, 0x1

    goto :goto_18

    :cond_38
    return-void
.end method

.method public final uncaughtException(Ljava/lang/Thread;Ljava/lang/Throwable;)V
    .registers 8

    const-string v0, "[Falcon_DiagMonSDK][1]["

    sget-object v1, Lb3/a;->a:Ljava/lang/String;

    new-instance v2, Ljava/lang/StringBuilder;

    const-string v3, "Agreement for ueHandler : "

    invoke-direct {v2, v3}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    iget-object v3, p0, LZ2/c;->e:LZ2/b;

    invoke-virtual {v3}, LZ2/b;->b()Z

    move-result v3

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(Z)Ljava/lang/StringBuilder;

    invoke-virtual {v2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v2

    invoke-static {v1, v2}, Landroid/util/Log;->d(Ljava/lang/String;Ljava/lang/String;)I

    iget-object v2, p0, LZ2/c;->e:LZ2/b;

    iget-object v3, v2, LZ2/b;->b:Ljava/lang/Object;

    check-cast v3, Landroid/content/Context;

    invoke-static {v3}, Lb3/a;->a(Landroid/content/Context;)I

    move-result v3

    const/4 v4, 0x1

    if-ne v3, v4, :cond_2f

    iget-object v2, v2, LZ2/b;->f:Ljava/lang/Object;

    check-cast v2, LZ2/a;

    iget-object v2, v2, LZ2/a;->a:Ljava/lang/String;

    goto :goto_33

    :cond_2f
    iget-object v2, v2, LZ2/b;->e:Ljava/lang/Object;

    check-cast v2, Ljava/lang/String;

    :goto_33
    const-string v3, "Agreement for ueHandler : "

    invoke-virtual {v3, v2}, Ljava/lang/String;->concat(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-static {v1, v2}, Landroid/util/Log;->d(Ljava/lang/String;Ljava/lang/String;)I

    new-instance v1, Ljava/lang/StringBuilder;

    const-string v2, "[Falcon_DiagMonSDK][0]["

    invoke-direct {v1, v2}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    const-string v2, "c"

    invoke-virtual {v1, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v2, "]"

    invoke-virtual {v1, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v1

    invoke-static {v1}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    :try_start_54
    iget-object v1, p0, LZ2/c;->e:LZ2/b;

    invoke-virtual {v1}, LZ2/b;->b()Z

    move-result v1

    if-eqz v1, :cond_10a

    invoke-static {}, Lb3/a;->b()Z

    move-result v1

    if-nez v1, :cond_10a

    new-instance v1, Ljava/lang/StringBuilder;

    invoke-direct {v1, v0}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    const-string v0, "c"

    invoke-virtual {v1, v0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v0, "]"

    invoke-virtual {v1, v0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    invoke-static {v0}, Lcom/bumptech/glide/c;->k(Ljava/lang/String;)V

    iget-object v0, p0, LZ2/c;->e:LZ2/b;

    iget-object v1, v0, LZ2/b;->b:Ljava/lang/Object;

    check-cast v1, Landroid/content/Context;

    iget-object v0, v0, LZ2/b;->c:Ljava/lang/Object;

    check-cast v0, Ljava/lang/String;

    invoke-static {v1, v0}, Lcom/bumptech/glide/c;->B(Landroid/content/Context;Ljava/lang/String;)V

    invoke-virtual {p0}, LZ2/c;->f()V

    iget-object v0, p0, LZ2/c;->b:Ljava/lang/String;

    const-string v1, "diagmon.log"

    invoke-static {v0, v1}, LZ2/c;->d(Ljava/lang/String;Ljava/lang/String;)Ljava/io/File;

    move-result-object v0

    const/4 v1, 0x0

    invoke-virtual {p0, v0, p2, v1}, LZ2/c;->e(Ljava/io/File;Ljava/lang/Throwable;Ljava/lang/String;)V

    iget-object v0, p0, LZ2/c;->b:Ljava/lang/String;

    const-string v1, "diagmon_event.log"

    invoke-static {v0, v1}, LZ2/c;->d(Ljava/lang/String;Ljava/lang/String;)Ljava/io/File;

    move-result-object v0

    iget-object v1, p0, LZ2/c;->c:Landroid/content/Context;

    iget-object v2, p0, LZ2/c;->a:[Ljava/lang/String;

    const/4 v3, 0x0

    aget-object v2, v2, v3

    invoke-static {v1, v2}, LZ2/c;->c(Landroid/content/Context;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-virtual {p0, v0, p2, v1}, LZ2/c;->e(Ljava/io/File;Ljava/lang/Throwable;Ljava/lang/String;)V

    iget-object v0, p0, LZ2/c;->b:Ljava/lang/String;

    const-string v1, "diagmon_thread.log"

    invoke-static {v0, v1}, LZ2/c;->d(Ljava/lang/String;Ljava/lang/String;)Ljava/io/File;

    move-result-object v0

    invoke-static {}, LZ2/c;->b()Ljava/lang/String;

    move-result-object v1

    invoke-virtual {p0, v0, p2, v1}, LZ2/c;->e(Ljava/io/File;Ljava/lang/Throwable;Ljava/lang/String;)V

    iget-object v0, p0, LZ2/c;->b:Ljava/lang/String;

    const-string v1, "diagmon_memory.log"

    invoke-static {v0, v1}, LZ2/c;->d(Ljava/lang/String;Ljava/lang/String;)Ljava/io/File;

    move-result-object v0

    iget-object v1, p0, LZ2/c;->c:Landroid/content/Context;

    iget-object v2, p0, LZ2/c;->a:[Ljava/lang/String;

    aget-object v2, v2, v4

    invoke-static {v1, v2}, LZ2/c;->c(Landroid/content/Context;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-virtual {p0, v0, p2, v1}, LZ2/c;->e(Ljava/io/File;Ljava/lang/Throwable;Ljava/lang/String;)V

    iget-object v0, p0, LZ2/c;->b:Ljava/lang/String;

    const-string v1, "diagmon_storage.log"

    invoke-static {v0, v1}, LZ2/c;->d(Ljava/lang/String;Ljava/lang/String;)Ljava/io/File;

    move-result-object v0

    iget-object v1, p0, LZ2/c;->c:Landroid/content/Context;

    iget-object v2, p0, LZ2/c;->a:[Ljava/lang/String;

    const/4 v3, 0x2

    aget-object v2, v2, v3

    invoke-static {v1, v2}, LZ2/c;->c(Landroid/content/Context;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-virtual {p0, v0, p2, v1}, LZ2/c;->e(Ljava/io/File;Ljava/lang/Throwable;Ljava/lang/String;)V

    iget-object v0, p0, LZ2/c;->c:Landroid/content/Context;

    invoke-static {v0}, Lb3/a;->a(Landroid/content/Context;)I

    move-result v0

    if-ne v0, v4, :cond_f7

    iget-object v0, p0, LZ2/c;->f:LA4/r;

    iget-object v1, p0, LZ2/c;->b:Ljava/lang/String;

    iput-object v1, v0, LA4/r;->b:Ljava/lang/Object;

    goto :goto_f7

    :catchall_f3
    move-exception v0

    goto :goto_119

    :catch_f5
    move-exception v0

    goto :goto_110

    :cond_f7
    :goto_f7
    invoke-virtual {p0}, LZ2/c;->g()V

    invoke-virtual {p0}, LZ2/c;->a()V

    monitor-enter p0
    :try_end_fe
    .catch Ljava/lang/OutOfMemoryError; {:try_start_54 .. :try_end_fe} :catch_f5
    .catchall {:try_start_54 .. :try_end_fe} :catchall_f3

    const-wide/16 v0, 0xbb8

    :try_start_100
    invoke-virtual {p0, v0, v1}, Ljava/lang/Object;->wait(J)V
    :try_end_103
    .catch Ljava/lang/Exception; {:try_start_100 .. :try_end_103} :catch_106
    .catchall {:try_start_100 .. :try_end_103} :catchall_104

    goto :goto_106

    :catchall_104
    move-exception v0

    goto :goto_108

    :catch_106
    :goto_106
    :try_start_106
    monitor-exit p0

    goto :goto_10a

    :goto_108
    monitor-exit p0
    :try_end_109
    .catchall {:try_start_106 .. :try_end_109} :catchall_104

    :try_start_109
    throw v0
    :try_end_10a
    .catch Ljava/lang/OutOfMemoryError; {:try_start_109 .. :try_end_10a} :catch_f5
    .catchall {:try_start_109 .. :try_end_10a} :catchall_f3

    :cond_10a
    :goto_10a
    iget-object p0, p0, LZ2/c;->d:Ljava/lang/Thread$UncaughtExceptionHandler;

    invoke-interface {p0, p1, p2}, Ljava/lang/Thread$UncaughtExceptionHandler;->uncaughtException(Ljava/lang/Thread;Ljava/lang/Throwable;)V

    goto :goto_118

    :goto_110
    :try_start_110
    invoke-virtual {v0}, Ljava/lang/Throwable;->getMessage()Ljava/lang/String;

    move-result-object v0

    invoke-static {v0}, Lcom/bumptech/glide/c;->o(Ljava/lang/String;)V
    :try_end_117
    .catchall {:try_start_110 .. :try_end_117} :catchall_f3

    goto :goto_10a

    :goto_118
    return-void

    :goto_119
    iget-object p0, p0, LZ2/c;->d:Ljava/lang/Thread$UncaughtExceptionHandler;

    invoke-interface {p0, p1, p2}, Ljava/lang/Thread$UncaughtExceptionHandler;->uncaughtException(Ljava/lang/Thread;Ljava/lang/Throwable;)V

    throw v0
.end method
