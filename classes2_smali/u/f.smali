.class public final Lu/f;
.super Lu/c;
.source "SourceFile"


# instance fields
.field public final synthetic C:I

.field public final D:Ln/a;

.field public E:Lp/p;

.field public final F:Landroid/os/Parcelable;

.field public final G:Ljava/lang/Object;

.field public final H:Ljava/lang/Object;

.field public I:Ljava/lang/Object;


# direct methods
.method public constructor <init>(Lm/x;Lu/i;I)V
    .registers 6

    iput p3, p0, Lu/f;->C:I

    packed-switch p3, :pswitch_data_62

    invoke-direct {p0, p1, p2}, Lu/c;-><init>(Lm/x;Lu/i;)V

    new-instance p3, Ln/a;

    const/4 v0, 0x3

    const/4 v1, 0x0

    invoke-direct {p3, v0, v1}, Ln/a;-><init>(II)V

    iput-object p3, p0, Lu/f;->D:Ln/a;

    new-instance p3, Landroid/graphics/Rect;

    invoke-direct {p3}, Landroid/graphics/Rect;-><init>()V

    iput-object p3, p0, Lu/f;->F:Landroid/os/Parcelable;

    new-instance p3, Landroid/graphics/Rect;

    invoke-direct {p3}, Landroid/graphics/Rect;-><init>()V

    iput-object p3, p0, Lu/f;->G:Ljava/lang/Object;

    iget-object p1, p1, Lm/x;->a:Lm/k;

    if-nez p1, :cond_25

    const/4 p1, 0x0

    goto :goto_2f

    :cond_25
    iget-object p1, p1, Lm/k;->d:Ljava/util/HashMap;

    iget-object p2, p2, Lu/i;->g:Ljava/lang/String;

    invoke-virtual {p1, p2}, Ljava/util/HashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object p1

    check-cast p1, Lm/y;

    :goto_2f
    iput-object p1, p0, Lu/f;->H:Ljava/lang/Object;

    return-void

    :pswitch_32  #0x1
    invoke-direct {p0, p1, p2}, Lu/c;-><init>(Lm/x;Lu/i;)V

    new-instance p1, Landroid/graphics/RectF;

    invoke-direct {p1}, Landroid/graphics/RectF;-><init>()V

    iput-object p1, p0, Lu/f;->F:Landroid/os/Parcelable;

    new-instance p1, Ln/a;

    invoke-direct {p1}, Ln/a;-><init>()V

    iput-object p1, p0, Lu/f;->D:Ln/a;

    const/16 p3, 0x8

    new-array p3, p3, [F

    iput-object p3, p0, Lu/f;->G:Ljava/lang/Object;

    new-instance p3, Landroid/graphics/Path;

    invoke-direct {p3}, Landroid/graphics/Path;-><init>()V

    iput-object p3, p0, Lu/f;->H:Ljava/lang/Object;

    iput-object p2, p0, Lu/f;->I:Ljava/lang/Object;

    const/4 p0, 0x0

    invoke-virtual {p1, p0}, Ln/a;->setAlpha(I)V

    sget-object p0, Landroid/graphics/Paint$Style;->FILL:Landroid/graphics/Paint$Style;

    invoke-virtual {p1, p0}, Landroid/graphics/Paint;->setStyle(Landroid/graphics/Paint$Style;)V

    iget p0, p2, Lu/i;->l:I

    invoke-virtual {p1, p0}, Landroid/graphics/Paint;->setColor(I)V

    return-void

    nop

    :pswitch_data_62
    .packed-switch 0x1
        :pswitch_32  #00000001
    .end packed-switch
.end method


# virtual methods
.method public final d(Ljava/lang/Object;Lz/c;)V
    .registers 5

    iget v0, p0, Lu/f;->C:I

    packed-switch v0, :pswitch_data_32

    invoke-super {p0, p1, p2}, Lu/c;->d(Ljava/lang/Object;Lz/c;)V

    sget-object v0, Lm/A;->F:Landroid/graphics/ColorFilter;

    if-ne p1, v0, :cond_14

    new-instance p1, Lp/p;

    const/4 v0, 0x0

    invoke-direct {p1, v0, p2}, Lp/p;-><init>(Ljava/lang/Object;Lz/c;)V

    iput-object p1, p0, Lu/f;->E:Lp/p;

    :cond_14
    return-void

    :pswitch_15  #0x0
    invoke-super {p0, p1, p2}, Lu/c;->d(Ljava/lang/Object;Lz/c;)V

    sget-object v0, Lm/A;->F:Landroid/graphics/ColorFilter;

    const/4 v1, 0x0

    if-ne p1, v0, :cond_25

    new-instance p1, Lp/p;

    invoke-direct {p1, v1, p2}, Lp/p;-><init>(Ljava/lang/Object;Lz/c;)V

    iput-object p1, p0, Lu/f;->E:Lp/p;

    goto :goto_30

    :cond_25
    sget-object v0, Lm/A;->I:Landroid/graphics/Bitmap;

    if-ne p1, v0, :cond_30

    new-instance p1, Lp/p;

    invoke-direct {p1, v1, p2}, Lp/p;-><init>(Ljava/lang/Object;Lz/c;)V

    iput-object p1, p0, Lu/f;->I:Ljava/lang/Object;

    :cond_30
    :goto_30
    return-void

    nop

    :pswitch_data_32
    .packed-switch 0x0
        :pswitch_15  #00000000
    .end packed-switch
.end method

.method public final e(Landroid/graphics/RectF;Landroid/graphics/Matrix;Z)V
    .registers 6

    iget v0, p0, Lu/f;->C:I

    packed-switch v0, :pswitch_data_42

    invoke-super {p0, p1, p2, p3}, Lu/c;->e(Landroid/graphics/RectF;Landroid/graphics/Matrix;Z)V

    iget-object p2, p0, Lu/f;->F:Landroid/os/Parcelable;

    check-cast p2, Landroid/graphics/RectF;

    iget-object p3, p0, Lu/f;->I:Ljava/lang/Object;

    check-cast p3, Lu/i;

    iget v0, p3, Lu/i;->j:I

    int-to-float v0, v0

    iget p3, p3, Lu/i;->k:I

    int-to-float p3, p3

    const/4 v1, 0x0

    invoke-virtual {p2, v1, v1, v0, p3}, Landroid/graphics/RectF;->set(FFFF)V

    iget-object p0, p0, Lu/c;->n:Landroid/graphics/Matrix;

    invoke-virtual {p0, p2}, Landroid/graphics/Matrix;->mapRect(Landroid/graphics/RectF;)Z

    invoke-virtual {p1, p2}, Landroid/graphics/RectF;->set(Landroid/graphics/RectF;)V

    return-void

    :pswitch_23  #0x0
    invoke-super {p0, p1, p2, p3}, Lu/c;->e(Landroid/graphics/RectF;Landroid/graphics/Matrix;Z)V

    iget-object p2, p0, Lu/f;->H:Ljava/lang/Object;

    check-cast p2, Lm/y;

    if-eqz p2, :cond_41

    invoke-static {}, Ly/f;->c()F

    move-result p3

    iget v0, p2, Lm/y;->a:I

    int-to-float v0, v0

    mul-float/2addr v0, p3

    iget p2, p2, Lm/y;->b:I

    int-to-float p2, p2

    mul-float/2addr p2, p3

    const/4 p3, 0x0

    invoke-virtual {p1, p3, p3, v0, p2}, Landroid/graphics/RectF;->set(FFFF)V

    iget-object p0, p0, Lu/c;->n:Landroid/graphics/Matrix;

    invoke-virtual {p0, p1}, Landroid/graphics/Matrix;->mapRect(Landroid/graphics/RectF;)Z

    :cond_41
    return-void

    :pswitch_data_42
    .packed-switch 0x0
        :pswitch_23  #00000000
    .end packed-switch
.end method

.method public final j(Landroid/graphics/Canvas;Landroid/graphics/Matrix;I)V
    .registers 20

    move-object/from16 v1, p0

    move-object/from16 v2, p1

    move/from16 v3, p3

    const/4 v0, 0x1

    const/4 v4, 0x0

    iget v5, v1, Lu/f;->C:I

    packed-switch v5, :pswitch_data_248

    iget-object v5, v1, Lu/f;->I:Ljava/lang/Object;

    check-cast v5, Lu/i;

    iget v6, v5, Lu/i;->l:I

    invoke-static {v6}, Landroid/graphics/Color;->alpha(I)I

    move-result v6

    if-nez v6, :cond_1b

    goto/16 :goto_a6

    :cond_1b
    iget-object v7, v1, Lu/c;->w:Lp/o;

    iget-object v7, v7, Lp/o;->j:Lp/d;

    if-nez v7, :cond_24

    const/16 v7, 0x64

    goto :goto_2e

    :cond_24
    invoke-virtual {v7}, Lp/d;->f()Ljava/lang/Object;

    move-result-object v7

    check-cast v7, Ljava/lang/Integer;

    invoke-virtual {v7}, Ljava/lang/Integer;->intValue()I

    move-result v7

    :goto_2e
    int-to-float v3, v3

    const/high16 v8, 0x437f0000  # 255.0f

    div-float/2addr v3, v8

    int-to-float v6, v6

    div-float/2addr v6, v8

    int-to-float v7, v7

    mul-float/2addr v6, v7

    const/high16 v7, 0x42c80000  # 100.0f

    div-float/2addr v6, v7

    mul-float/2addr v6, v3

    mul-float/2addr v6, v8

    float-to-int v3, v6

    iget-object v6, v1, Lu/f;->D:Ln/a;

    invoke-virtual {v6, v3}, Ln/a;->setAlpha(I)V

    iget-object v7, v1, Lu/f;->E:Lp/p;

    if-eqz v7, :cond_4e

    invoke-virtual {v7}, Lp/p;->f()Ljava/lang/Object;

    move-result-object v7

    check-cast v7, Landroid/graphics/ColorFilter;

    invoke-virtual {v6, v7}, Landroid/graphics/Paint;->setColorFilter(Landroid/graphics/ColorFilter;)Landroid/graphics/ColorFilter;

    :cond_4e
    if-lez v3, :cond_a6

    iget-object v3, v1, Lu/f;->G:Ljava/lang/Object;

    check-cast v3, [F

    const/4 v7, 0x0

    aput v7, v3, v4

    aput v7, v3, v0

    iget v8, v5, Lu/i;->j:I

    int-to-float v8, v8

    const/4 v9, 0x2

    aput v8, v3, v9

    const/4 v10, 0x3

    aput v7, v3, v10

    const/4 v11, 0x4

    aput v8, v3, v11

    iget v5, v5, Lu/i;->k:I

    int-to-float v5, v5

    const/4 v8, 0x5

    aput v5, v3, v8

    const/4 v12, 0x6

    aput v7, v3, v12

    const/4 v7, 0x7

    aput v5, v3, v7

    move-object/from16 v5, p2

    invoke-virtual {v5, v3}, Landroid/graphics/Matrix;->mapPoints([F)V

    iget-object v1, v1, Lu/f;->H:Ljava/lang/Object;

    check-cast v1, Landroid/graphics/Path;

    invoke-virtual {v1}, Landroid/graphics/Path;->reset()V

    aget v5, v3, v4

    aget v13, v3, v0

    invoke-virtual {v1, v5, v13}, Landroid/graphics/Path;->moveTo(FF)V

    aget v5, v3, v9

    aget v9, v3, v10

    invoke-virtual {v1, v5, v9}, Landroid/graphics/Path;->lineTo(FF)V

    aget v5, v3, v11

    aget v8, v3, v8

    invoke-virtual {v1, v5, v8}, Landroid/graphics/Path;->lineTo(FF)V

    aget v5, v3, v12

    aget v7, v3, v7

    invoke-virtual {v1, v5, v7}, Landroid/graphics/Path;->lineTo(FF)V

    aget v4, v3, v4

    aget v0, v3, v0

    invoke-virtual {v1, v4, v0}, Landroid/graphics/Path;->lineTo(FF)V

    invoke-virtual {v1}, Landroid/graphics/Path;->close()V

    invoke-virtual {v2, v1, v6}, Landroid/graphics/Canvas;->drawPath(Landroid/graphics/Path;Landroid/graphics/Paint;)V

    :cond_a6
    :goto_a6
    return-void

    :pswitch_a7  #0x0
    move-object/from16 v5, p2

    iget-object v6, v1, Lu/f;->I:Ljava/lang/Object;

    check-cast v6, Lp/p;

    iget-object v7, v1, Lu/c;->o:Lm/x;

    iget-object v8, v1, Lu/f;->H:Ljava/lang/Object;

    check-cast v8, Lm/y;

    if-eqz v6, :cond_bf

    invoke-virtual {v6}, Lp/p;->f()Ljava/lang/Object;

    move-result-object v6

    check-cast v6, Landroid/graphics/Bitmap;

    if-eqz v6, :cond_bf

    goto/16 :goto_1e1

    :cond_bf
    iget-object v6, v1, Lu/c;->p:Lu/i;

    iget-object v6, v6, Lu/i;->g:Ljava/lang/String;

    invoke-virtual {v7}, Landroid/graphics/drawable/Drawable;->getCallback()Landroid/graphics/drawable/Drawable$Callback;

    move-result-object v9

    const/4 v10, 0x0

    if-nez v9, :cond_cc

    move-object v9, v10

    goto :goto_108

    :cond_cc
    iget-object v9, v7, Lm/x;->h:Lq/a;

    if-eqz v9, :cond_f1

    invoke-virtual {v7}, Landroid/graphics/drawable/Drawable;->getCallback()Landroid/graphics/drawable/Drawable$Callback;

    move-result-object v11

    if-nez v11, :cond_d8

    :cond_d6
    move-object v11, v10

    goto :goto_e2

    :cond_d8
    instance-of v12, v11, Landroid/view/View;

    if-eqz v12, :cond_d6

    check-cast v11, Landroid/view/View;

    invoke-virtual {v11}, Landroid/view/View;->getContext()Landroid/content/Context;

    move-result-object v11

    :goto_e2
    iget-object v9, v9, Lq/a;->a:Landroid/content/Context;

    if-nez v11, :cond_e8

    if-eqz v9, :cond_f1

    :cond_e8
    invoke-virtual {v9, v11}, Ljava/lang/Object;->equals(Ljava/lang/Object;)Z

    move-result v9

    if-eqz v9, :cond_ef

    goto :goto_f1

    :cond_ef
    iput-object v10, v7, Lm/x;->h:Lq/a;

    :cond_f1
    :goto_f1
    iget-object v9, v7, Lm/x;->h:Lq/a;

    if-nez v9, :cond_106

    new-instance v9, Lq/a;

    invoke-virtual {v7}, Landroid/graphics/drawable/Drawable;->getCallback()Landroid/graphics/drawable/Drawable$Callback;

    move-result-object v11

    iget-object v12, v7, Lm/x;->i:Ljava/lang/String;

    iget-object v13, v7, Lm/x;->a:Lm/k;

    iget-object v13, v13, Lm/k;->d:Ljava/util/HashMap;

    invoke-direct {v9, v11, v12, v13}, Lq/a;-><init>(Landroid/graphics/drawable/Drawable$Callback;Ljava/lang/String;Ljava/util/HashMap;)V

    iput-object v9, v7, Lm/x;->h:Lq/a;

    :cond_106
    iget-object v9, v7, Lm/x;->h:Lq/a;

    :goto_108
    if-eqz v9, :cond_1d7

    iget-object v11, v9, Lq/a;->b:Ljava/lang/String;

    iget-object v12, v9, Lq/a;->c:Ljava/util/HashMap;

    invoke-virtual {v12, v6}, Ljava/util/HashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v12

    check-cast v12, Lm/y;

    if-nez v12, :cond_119

    :goto_116
    move-object v13, v10

    goto/16 :goto_1d5

    :cond_119
    iget-object v13, v12, Lm/y;->d:Landroid/graphics/Bitmap;

    if-eqz v13, :cond_11f

    goto/16 :goto_1d5

    :cond_11f
    new-instance v13, Landroid/graphics/BitmapFactory$Options;

    invoke-direct {v13}, Landroid/graphics/BitmapFactory$Options;-><init>()V

    iput-boolean v0, v13, Landroid/graphics/BitmapFactory$Options;->inScaled:Z

    const/16 v14, 0xa0

    iput v14, v13, Landroid/graphics/BitmapFactory$Options;->inDensity:I

    iget-object v14, v12, Lm/y;->c:Ljava/lang/String;

    const-string v15, "data:"

    invoke-virtual {v14, v15}, Ljava/lang/String;->startsWith(Ljava/lang/String;)Z

    move-result v15

    if-eqz v15, :cond_16a

    const-string v15, "base64,"

    invoke-virtual {v14, v15}, Ljava/lang/String;->indexOf(Ljava/lang/String;)I

    move-result v15

    if-lez v15, :cond_16a

    const/16 v11, 0x2c

    :try_start_13e
    invoke-virtual {v14, v11}, Ljava/lang/String;->indexOf(I)I

    move-result v11

    add-int/2addr v11, v0

    invoke-virtual {v14, v11}, Ljava/lang/String;->substring(I)Ljava/lang/String;

    move-result-object v0

    invoke-static {v0, v4}, Landroid/util/Base64;->decode(Ljava/lang/String;I)[B

    move-result-object v0
    :try_end_14b
    .catch Ljava/lang/IllegalArgumentException; {:try_start_13e .. :try_end_14b} :catch_163

    array-length v11, v0

    invoke-static {v0, v4, v11, v13}, Landroid/graphics/BitmapFactory;->decodeByteArray([BIILandroid/graphics/BitmapFactory$Options;)Landroid/graphics/Bitmap;

    move-result-object v13

    sget-object v11, Lq/a;->d:Ljava/lang/Object;

    monitor-enter v11

    :try_start_153
    iget-object v0, v9, Lq/a;->c:Ljava/util/HashMap;

    invoke-virtual {v0, v6}, Ljava/util/HashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lm/y;

    iput-object v13, v0, Lm/y;->d:Landroid/graphics/Bitmap;

    monitor-exit v11

    goto/16 :goto_1d5

    :catchall_160
    move-exception v0

    monitor-exit v11
    :try_end_162
    .catchall {:try_start_153 .. :try_end_162} :catchall_160

    throw v0

    :catch_163
    move-exception v0

    const-string v6, "data URL did not have correct base64 format."

    invoke-static {v6, v0}, Ly/b;->c(Ljava/lang/String;Ljava/lang/Throwable;)V

    goto :goto_116

    :cond_16a
    :try_start_16a
    invoke-static {v11}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v15

    if-nez v15, :cond_1c6

    iget-object v15, v9, Lq/a;->a:Landroid/content/Context;

    invoke-virtual {v15}, Landroid/content/Context;->getAssets()Landroid/content/res/AssetManager;

    move-result-object v15

    new-instance v4, Ljava/lang/StringBuilder;

    invoke-direct {v4}, Ljava/lang/StringBuilder;-><init>()V

    invoke-virtual {v4, v11}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v4, v14}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v4}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v4

    invoke-virtual {v15, v4}, Landroid/content/res/AssetManager;->open(Ljava/lang/String;)Ljava/io/InputStream;

    move-result-object v4
    :try_end_189
    .catch Ljava/io/IOException; {:try_start_16a .. :try_end_189} :catch_1c4

    :try_start_189
    invoke-static {v4, v10, v13}, Landroid/graphics/BitmapFactory;->decodeStream(Ljava/io/InputStream;Landroid/graphics/Rect;Landroid/graphics/BitmapFactory$Options;)Landroid/graphics/Bitmap;

    move-result-object v4
    :try_end_18d
    .catch Ljava/lang/IllegalArgumentException; {:try_start_189 .. :try_end_18d} :catch_1bb

    iget v11, v12, Lm/y;->a:I

    iget v12, v12, Lm/y;->b:I

    sget-object v13, Ly/f;->a:LN3/b;

    invoke-virtual {v4}, Landroid/graphics/Bitmap;->getWidth()I

    move-result v13

    if-ne v13, v11, :cond_1a1

    invoke-virtual {v4}, Landroid/graphics/Bitmap;->getHeight()I

    move-result v13

    if-ne v13, v12, :cond_1a1

    move-object v13, v4

    goto :goto_1a9

    :cond_1a1
    invoke-static {v4, v11, v12, v0}, Landroid/graphics/Bitmap;->createScaledBitmap(Landroid/graphics/Bitmap;IIZ)Landroid/graphics/Bitmap;

    move-result-object v0

    invoke-virtual {v4}, Landroid/graphics/Bitmap;->recycle()V

    move-object v13, v0

    :goto_1a9
    sget-object v4, Lq/a;->d:Ljava/lang/Object;

    monitor-enter v4

    :try_start_1ac
    iget-object v0, v9, Lq/a;->c:Ljava/util/HashMap;

    invoke-virtual {v0, v6}, Ljava/util/HashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lm/y;

    iput-object v13, v0, Lm/y;->d:Landroid/graphics/Bitmap;

    monitor-exit v4

    goto :goto_1d5

    :catchall_1b8
    move-exception v0

    monitor-exit v4
    :try_end_1ba
    .catchall {:try_start_1ac .. :try_end_1ba} :catchall_1b8

    throw v0

    :catch_1bb
    move-exception v0

    move-object v4, v0

    const-string v0, "Unable to decode image."

    invoke-static {v0, v4}, Ly/b;->c(Ljava/lang/String;Ljava/lang/Throwable;)V

    goto/16 :goto_116

    :catch_1c4
    move-exception v0

    goto :goto_1ce

    :cond_1c6
    :try_start_1c6
    new-instance v0, Ljava/lang/IllegalStateException;

    const-string v4, "You must set an images folder before loading an image. Set it with LottieComposition#setImagesFolder or LottieDrawable#setImagesFolder"

    invoke-direct {v0, v4}, Ljava/lang/IllegalStateException;-><init>(Ljava/lang/String;)V

    throw v0
    :try_end_1ce
    .catch Ljava/io/IOException; {:try_start_1c6 .. :try_end_1ce} :catch_1c4

    :goto_1ce
    const-string v4, "Unable to open asset."

    invoke-static {v4, v0}, Ly/b;->c(Ljava/lang/String;Ljava/lang/Throwable;)V

    goto/16 :goto_116

    :goto_1d5
    move-object v6, v13

    goto :goto_1d8

    :cond_1d7
    move-object v6, v10

    :goto_1d8
    if-eqz v6, :cond_1db

    goto :goto_1e1

    :cond_1db
    if-eqz v8, :cond_1e0

    iget-object v6, v8, Lm/y;->d:Landroid/graphics/Bitmap;

    goto :goto_1e1

    :cond_1e0
    move-object v6, v10

    :goto_1e1
    if-eqz v6, :cond_247

    invoke-virtual {v6}, Landroid/graphics/Bitmap;->isRecycled()Z

    move-result v0

    if-nez v0, :cond_247

    if-nez v8, :cond_1ec

    goto :goto_247

    :cond_1ec
    invoke-static {}, Ly/f;->c()F

    move-result v0

    iget-object v4, v1, Lu/f;->D:Ln/a;

    invoke-virtual {v4, v3}, Ln/a;->setAlpha(I)V

    iget-object v3, v1, Lu/f;->E:Lp/p;

    if-eqz v3, :cond_202

    invoke-virtual {v3}, Lp/p;->f()Ljava/lang/Object;

    move-result-object v3

    check-cast v3, Landroid/graphics/ColorFilter;

    invoke-virtual {v4, v3}, Landroid/graphics/Paint;->setColorFilter(Landroid/graphics/ColorFilter;)Landroid/graphics/ColorFilter;

    :cond_202
    invoke-virtual/range {p1 .. p1}, Landroid/graphics/Canvas;->save()I

    invoke-virtual/range {p1 .. p2}, Landroid/graphics/Canvas;->concat(Landroid/graphics/Matrix;)V

    invoke-virtual {v6}, Landroid/graphics/Bitmap;->getWidth()I

    move-result v3

    invoke-virtual {v6}, Landroid/graphics/Bitmap;->getHeight()I

    move-result v5

    iget-object v9, v1, Lu/f;->F:Landroid/os/Parcelable;

    check-cast v9, Landroid/graphics/Rect;

    const/4 v10, 0x0

    invoke-virtual {v9, v10, v10, v3, v5}, Landroid/graphics/Rect;->set(IIII)V

    iget-boolean v3, v7, Lm/x;->l:Z

    iget-object v1, v1, Lu/f;->G:Ljava/lang/Object;

    check-cast v1, Landroid/graphics/Rect;

    if-eqz v3, :cond_22f

    iget v3, v8, Lm/y;->a:I

    int-to-float v3, v3

    mul-float/2addr v3, v0

    float-to-int v3, v3

    iget v5, v8, Lm/y;->b:I

    int-to-float v5, v5

    mul-float/2addr v5, v0

    float-to-int v0, v5

    const/4 v5, 0x0

    invoke-virtual {v1, v5, v5, v3, v0}, Landroid/graphics/Rect;->set(IIII)V

    goto :goto_241

    :cond_22f
    const/4 v5, 0x0

    invoke-virtual {v6}, Landroid/graphics/Bitmap;->getWidth()I

    move-result v3

    int-to-float v3, v3

    mul-float/2addr v3, v0

    float-to-int v3, v3

    invoke-virtual {v6}, Landroid/graphics/Bitmap;->getHeight()I

    move-result v7

    int-to-float v7, v7

    mul-float/2addr v7, v0

    float-to-int v0, v7

    invoke-virtual {v1, v5, v5, v3, v0}, Landroid/graphics/Rect;->set(IIII)V

    :goto_241
    invoke-virtual {v2, v6, v9, v1, v4}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;Landroid/graphics/Rect;Landroid/graphics/Rect;Landroid/graphics/Paint;)V

    invoke-virtual/range {p1 .. p1}, Landroid/graphics/Canvas;->restore()V

    :cond_247
    :goto_247
    return-void

    :pswitch_data_248
    .packed-switch 0x0
        :pswitch_a7  #00000000
    .end packed-switch
.end method
